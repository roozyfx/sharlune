...
TEST(TupleMathTest, Addable) {
  ...
  Vec4 v3 = v1 + v2;
  EXPECT_DOUBLE_EQ(v3.x, 6.0);
  EXPECT_DOUBLE_EQ(v3.y, 8.0);
  EXPECT_DOUBLE_EQ(v3.z, 10.0);
  EXPECT_DOUBLE_EQ(v3.w, 12.0);

  v1 += v2;
  EXPECT_DOUBLE_EQ(v1.x, 6.0);
  EXPECT_DOUBLE_EQ(v1.y, 8.0);
  EXPECT_DOUBLE_EQ(v1.z, 10.0);
...
TEST(TupleMathTest, Subtractable) {
  ...
  EXPECT_DOUBLE_EQ(v3.y, 18.0);
  EXPECT_DOUBLE_EQ(v3.z, 27.0);
  EXPECT_DOUBLE_EQ(v3.w, 36.0);

  v1 -= v2;
  EXPECT_DOUBLE_EQ(v1.x, 9.0);
  EXPECT_DOUBLE_EQ(v1.y, 18.0);
  EXPECT_DOUBLE_EQ(v1.z, 27.0);
  EXPECT_DOUBLE_EQ(v1.w, 36.0);
}
...
TEST(TupleMathTest, Multipliable) {
  ...
  Vec4 v_mul1 = v * 2.0;
  EXPECT_DOUBLE_EQ(v_mul1.x, 4.0);
  EXPECT_DOUBLE_EQ(v_mul1.y, 6.0);
  EXPECT_DOUBLE_EQ(v_mul1.z, 8.0);
  EXPECT_DOUBLE_EQ(v_mul1.w, 10.0);

  Vec4 v_mul2 = 3.0 * v;
  EXPECT_DOUBLE_EQ(v_mul2.x, 6.0);
  EXPECT_DOUBLE_EQ(v_mul2.y, 9.0);
  EXPECT_DOUBLE_EQ(v_mul2.z, 12.0);
  EXPECT_DOUBLE_EQ(v_mul2.w, 15.0);

  v *= 4.0;
  EXPECT_DOUBLE_EQ(v.x, 8.0);
  EXPECT_DOUBLE_EQ(v.y, 12.0);
  EXPECT_DOUBLE_EQ(v.z, 16.0);
  EXPECT_DOUBLE_EQ(v.w, 20.0);
}
...
TEST(TupleMathTest, Divideable) {
  ...
  Vec4 v_div = v / 2.0;
  EXPECT_DOUBLE_EQ(v_div.x, 2.0);
  EXPECT_DOUBLE_EQ(v_div.y, 4.0);
  EXPECT_DOUBLE_EQ(v_div.z, 6.0);
  EXPECT_DOUBLE_EQ(v_div.w, 8.0);
...
TEST(TupleMathTest, HasL2Distance) {
  Vec2 v2(3.0, 4.0);
  EXPECT_DOUBLE_EQ(length_squared(v2), 25.0);
  EXPECT_DOUBLE_EQ(length(v2), 5.0);

  Vec3 v3(2.0, 3.0, 6.0);
  EXPECT_DOUBLE_EQ(length_squared(v3), 49.0);  // 4 + 9 + 36 = 49
  EXPECT_DOUBLE_EQ(length(v3), 7.0);

  Vec4 v4(1.0, 2.0, 3.0, 4.0);
  EXPECT_DOUBLE_EQ(length_squared(v4), 30.0);  // 1 + 4 + 9 + 16 = 30
}
...
TEST(TupleMathTest, DotProductable) {
  ...
  EXPECT_DOUBLE_EQ(dot_product_3d, 32.0);
  ...
  EXPECT_DOUBLE_EQ(dot_product_4d, 40.0);
}
...
TEST(TupleMathTest, CrossProductable) {
  ...
  EXPECT_DOUBLE_EQ(neg_k.x, 0.0);
  EXPECT_DOUBLE_EQ(neg_k.y, 0.0);
  EXPECT_DOUBLE_EQ(neg_k.z, -1.0);
}
...
TEST(TupleMathTest, Normalize) {
  ...
  EXPECT_DOUBLE_EQ(v2_norm.x, 0.6);
  EXPECT_DOUBLE_EQ(v2_norm.y, 0.8);
  EXPECT_DOUBLE_EQ(length(v2_norm), 1.0);
  ...
  EXPECT_DOUBLE_EQ(v3_norm.x, 0.0);
  EXPECT_DOUBLE_EQ(v3_norm.y, 0.6);
  EXPECT_DOUBLE_EQ(v3_norm.z, 0.8);
  EXPECT_DOUBLE_EQ(length(v3_norm), 1.0);
  ...
  EXPECT_DOUBLE_EQ(v4_norm.x, 0.5);
  EXPECT_DOUBLE_EQ(v4_norm.y, 0.5);
